...
class QuestionMethodTests(TestCase):
	...
	def test_was_published_recently_with_recent_question(self):
		"""
		test_was_published_recently_with_recent_question() should return
...
def add_three_choices(question):
	for i in range(3):
		question.choice_set.create(choice_text="choice %s" %str(i),
								   votes=0)

class QuestionHasChoicesTest(TestCase):
	"""
	A Question should not be published if it hasn't got any choices.
	"""
	def test_question_with_no_choices(self):
		q_no_choices = create_question("q_no_choices", 0)
		response = self.client.get(reverse('polls:index'))
		self.assertContains(response, "No polls are available.")

	def test_question_with_1_choice(self):
		q_1_choice = create_question_with_choices("q_1_choice", 0)
		response = self.client.get(reverse('polls:index'))
		self.assertContains(response, "q_1_choice")